		...
		for (;;) {
			...
			if (gameType == AI_VS_AI) {
				if (m.getPossibleMoves(Othello.PLAYER1).size() != 0) {
					move = Search.MiniMax(new Node(m, null, Node.MAX, Othello.PLAYER2), Othello.PLAYER1, true, Search.HEURISTIC_MOBILITY, 4);
			...
			if (gameType == AI_VS_AI) {
				if (m.getPossibleMoves(Othello.PLAYER2).size() != 0) {
					move = Search.MiniMax(new Node(m, null, Node.MAX, Othello.PLAYER1), Othello.PLAYER2, true, Search.HEURISTIC_MOBILITY, 4);
					m.handleSelection(move.getRow(), move.getCol(), Othello.PLAYER2);
					gb.update(m);
				}
				try {
